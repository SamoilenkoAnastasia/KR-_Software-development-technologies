...
import ua.kpi.personal.repo.CategoryCache; // Припущено, що CategoryCache існує
...
// Більше не реалізує TransactionProcessor
public class TransactionDao {
    ...
    private final String SELECT_FIELDS = "t.id, t.amount, t.type, t.description, t.created_at, t.category_id, t.account_id, t.user_id, t.currency, t.template_id, t.trans_date, t.budget_id";
    ...
    public Transaction findById(Long id, Long budgetId) {
        String sql = "SELECT " + SELECT_FIELDS + " FROM transactions t WHERE t.id = ? AND t.budget_id = ?";
        try (Connection c = Db.getConnection();
             PreparedStatement ps = c.prepareStatement(sql)) {
             ps.setLong(1, id);
             ...
             try (ResultSet rs = ps.executeQuery()) {
                 if (rs.next()) {
                     List<Account> allAccounts = accountDao.findByBudgetId(budgetId);
                     return mapResultSetToTransaction(rs, allAccounts);
                 }
             }
        } catch (SQLException e) {
            e.printStackTrace();
